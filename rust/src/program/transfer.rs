...
mod tests {
    ...
    fn try_transfer(
        sender: &PrivateKey<TestnetV0>,
        recipient: &Address<TestnetV0>,
        amount: u64,
        ...
        let program_manager =
            ProgramManager::<TestnetV0>::new(Some(*sender), None, Some(api_client.clone()), None).unwrap();
    ...
    fn verify_transfer(
        amount: u64,
        api_client: &AleoAPIClient<TestnetV0>,
        recipient_private_key: &PrivateKey<TestnetV0>,
        visibility: TransferType,
    ) {
        for i in 0..10 {
    ...
    fn test_transfer_roundtrip() {
        ...
        let beacon_private_key = PrivateKey::<TestnetV0>::from_str(BEACON_PRIVATE_KEY).unwrap();
        ...
        let private_recipient_private_key =
            PrivateKey::<TestnetV0>::from_str("APrivateKey1zkpCF24vGLohfHWNZam1z7qDhDq5Bp1u8WPFhh9q2wWoNh8").unwrap();
        ...
        let private_to_public_recipient_private_key =
            PrivateKey::<TestnetV0>::from_str("APrivateKey1zkpFkojCEFsw3LaozkqaVkMuxdTq2DEsUV88WGexXoWGuNA").unwrap();
        ...
        let public_recipient_private_key =
            PrivateKey::<TestnetV0>::from_str("APrivateKey1zkp6rwhE5Jxz16cv32kM8Z8VMsLe78BCK8LU3FM7htmSsis").unwrap();
        ...
        let public_to_private_recipient_private_key =
            PrivateKey::<TestnetV0>::from_str("APrivateKey1zkp3NchSbrypyf2UoJSGyag58biAFPvtd1WtpM5M9pqoifK").unwrap();
        ...
        let api_client = AleoAPIClient::<TestnetV0>::local_testnet3("3030");
        let public_address_literal = Literal::<TestnetV0>::from_str(&public_recipient_address.to_string()).unwrap();
        let private_to_public_address_literal =
            Literal::<TestnetV0>::from_str(&private_to_public_recipient_address.to_string()).unwrap();
        let expected_value = Value::from(Plaintext::<TestnetV0>::from_str(amount_str).unwrap());
        let zero_value = Value::from(Plaintext::<TestnetV0>::from_str("0u64").unwrap());
