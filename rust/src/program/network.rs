...
mod tests {
    ...
    fn test_network_functionality_works_as_expected() {
        ...
        let program_manager =
            ProgramManager::<TestnetV0>::new(Some(private_key), None, Some(api_client), Some(temp_dir.clone())).unwrap();
        ...
        let wrong_hello_program = Program::<TestnetV0>::from_str(&wrong_hello_program_string).unwrap();
